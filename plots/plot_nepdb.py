...
# COLOR_OURS = '#E64B35'     
# COLOR_TRANSHLA = '#4DBBD5' 
# COLOR_SOTA_DEP = '#00A087' 
# COLOR_OTHER = '#B0B0B0'    
...
#     # ★ 修复1：使用 ax.get_xaxis_transform() 将Y坐标设为相对轴的百分比，避免撞击标题
#     ax.text(0.02, 0.95, 'Representation Collapse\n (MCC ≤ 0)', color='#E64B35', 
#             fontsize=10, fontweight='bold', va='top', ha='left', transform=ax.get_xaxis_transform(), zorder=2)
...
#     ax.set_xlabel('Matthews Correlation Coefficient (MCC)', fontweight='bold', fontsize=12)
...
#         display_label = model_info['label'] # ★ 修复2：提取展示用的名字
...
#         ax.plot(angles, means, color=color, linewidth=2.5, label=display_label, zorder=3)
...
# gs = GridSpec(2, 2, width_ratios=[1.2, 1], wspace=0.3, hspace=0.3)
...
# models_radar_hla1 = [
#     {'name': 'Our model', 'label': 'Our model', 'color': COLOR_OURS},
#     {'name': 'TransHLA', 'label': 'TransHLA', 'color': COLOR_TRANSHLA},
#     {'name': 'NetMHCPan4.1b', 'label': 'Top Allele-Dependent', 'color': COLOR_SOTA_DEP} # ★ 修复2：修改展示名字
# ]
...
#     {'name': 'Our model', 'label': 'Our model', 'color': COLOR_OURS},
#     {'name': 'TransHLA', 'label': 'TransHLA', 'color': COLOR_TRANSHLA},
#     {'name': 'MixMHC2pred', 'label': 'Top Allele-Dependent', 'color': COLOR_SOTA_DEP} # ★ 修复2：共用相同的展示名字
# ]
...
# # ==========================================
# forest_legend = [
#     mlines.Line2D([], [], color=COLOR_OURS, marker='s', linestyle='None', markersize=10, label='Our model (Allele-Agnostic)'),
#     mlines.Line2D([], [], color=COLOR_TRANSHLA, marker='s', linestyle='None', markersize=8, label='TransHLA (Allele-Agnostic)'),
#     mlines.Line2D([], [], color=COLOR_OTHER, marker='o', linestyle='None', markersize=8, label='Allele-Dependent Baselines')
# ]
# fig.legend(handles=forest_legend, loc='upper center', bbox_to_anchor=(0.3, 0.05), ncol=3, frameon=False, fontsize=11)

# handles, labels = ax2.get_legend_handles_labels()
# fig.legend(handles, labels, loc='upper center', bbox_to_anchor=(0.75, 0.05), ncol=3, frameon=False, fontsize=11)

# plt.tight_layout(rect=[0, 0.08, 1, 1])

# plt.savefig("plot_nepdb.pdf", format='pdf', dpi=300, bbox_inches='tight')
# plt.savefig("plot_nepdb.png", format='png', dpi=600, bbox_inches='tight')

# print("微调完成！标题避让完美，图例逻辑已自洽！")
# plt.show()
