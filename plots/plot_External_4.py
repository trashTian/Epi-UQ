...
# # NPG 经典配色
# COLORS = {
#     "Our model": "#E64B35",      # 深赤红
#     "TransHLA": "#4DBBD5",       # 蔚蓝
#     "Mhcflurry": "#00A087",      # 孔雀绿
#     "Mhcnuggets": "#3C5488",     # 藏青
#     "TransPHLA": "#F39B7F",      # 珊瑚橙
#     "Anthem": "#8491B4",         # 灰紫
#     "NetMHCPan4.1b": "#91D1C2",  # 浅青
#     "MixMHCpred": "#7E6148",     # 咖啡色
#     "DeepSeqPanII": "#B09C85",   # 浅卡其
#     "NetMHCIIpan4.3b": "#4E596F",# 深灰蓝
#     "MixMHC2pred": "#D4A373"     # 沙色
# }
...
# # ==========================================
# hla1_data = {
#     "Our model":      [0.643, 0.023, 0.911, 0.002, "Agnostic"],
...
# hla2_data = {
#     "Our model":      [0.510, 0.038, 0.758, 0.008, "Agnostic"],
...
# # ==========================================
# # ★ 全新调校：文本避让排版字典 (x_offset, y_offset, horizontal_alignment)
# # ==========================================
# TEXT_OFFSETS_HLA1 = {
#     "Our model": (0, -15, 'center'),     # 垂直正下方悬挂，完美避开顶部绿色标题
#     "TransHLA": (10, -12, 'left'),       # 向右下方偏移
#     "Mhcflurry": (10, 0, 'left'),
#     "NetMHCPan4.1b": (10, -8, 'left'),   # 微调避让
#     "MixMHCpred": (10, 5, 'left'),       
#     "Anthem": (10, -5, 'left'),          
#     "TransPHLA": (10, 0, 'left'),
#     "Mhcnuggets": (10, 0, 'left')
# }

# TEXT_OFFSETS_HLA2 = {
#     "Our model": (0, -15, 'center'),     # 垂直正下方悬挂
#     "TransHLA": (10, 0, 'left'),
#     "MixMHC2pred": (10, 0, 'left'),
#     "NetMHCIIpan4.3b": (10, 0, 'left'),  # 现在可以放心居中了，因为左边的字挪走了
#     "Mhcnuggets": (10, 0, 'left'),       
#     "DeepSeqPanII": (10, 0, 'left')      
# }
...
#     # 陷阱区 (红色)
#     rect_trap = patches.Rectangle((xlim[0], ylim[0]), prec_safe_threshold - xlim[0], ylim[1] - ylim[0], 
#                                   linewidth=0, facecolor='#FF0000', alpha=0.06, zorder=0)
#     ax.add_patch(rect_trap)
#     # 文字往上提了一点，防止碰到数据点
#     ax.text(xlim[0] + (prec_safe_threshold - xlim[0])/2, ylim[1] - (ylim[1]-ylim[0])*0.04, 
#             "Low Precision Regime", ha='center', va='top', fontsize=11, color='#B22222', fontweight='bold', zorder=1)
    
#     # 安全区 (绿色)
#     rect_safe = patches.Rectangle((prec_safe_threshold, ylim[0]), xlim[1] - prec_safe_threshold, ylim[1] - ylim[0], 
#                                   linewidth=0, facecolor='#008000', alpha=0.06, zorder=0)
#     ax.add_patch(rect_safe)
#     ax.text(prec_safe_threshold + (xlim[1] - prec_safe_threshold)/2, ylim[1] - (ylim[1]-ylim[0])*0.04, 
#             "High Precision Regime", ha='center', va='top', fontsize=11, color='#006400', fontweight='bold', zorder=1)

#     # ★ 关键修复：随机猜测线文字挪到了线的【左侧】
#     ax.axvline(x=0.2, color='#888888', linestyle='--', linewidth=1.5, zorder=1)
#     ax.text(0.195, ylim[0] + (ylim[1]-ylim[0])*0.1, 'Random Guess Baseline (0.2)', 
#             rotation=90, va='bottom', ha='right', color='#888888', fontsize=10, fontweight='bold')
...
#         size = 130 if model == "Our model" else 90
#         z_ord = 5 if model == "Our model" else 4
        
#         # 画点与线
#         ax.errorbar(prec_m, auc_m, xerr=prec_s, yerr=auc_s, fmt=marker_style, color=COLORS[model], 
#                     ecolor=COLORS[model], elinewidth=1.5, capsize=0, markersize=size/10, 
#                     markeredgecolor='white', markeredgewidth=1.2, zorder=z_ord, label=model)
        
#         # 添加文本标注
#         x_off, y_off, h_align = offsets_dict[model]
#         font_weight = 'bold' if model == "Our model" else 'medium'
        
#         ax.annotate(model, (prec_m, auc_m), 
#                     xytext=(x_off, y_off), textcoords='offset points',
#                     ha=h_align, va='center' if y_off == 0 else 'top', # 如果文字在下方，按顶部对齐
#                     fontsize=10, fontweight=font_weight, 
#                     color='#333333', zorder=6)

#     ax.set_xlabel('Precision (Suppressing False Positives)', fontweight='bold')
#     ax.set_ylabel('AUC (Overall Discrimination)', fontweight='bold')
...
# order = ["Our model", "TransHLA", "Mhcflurry", "NetMHCPan4.1b", "MixMHCpred", "Anthem", "TransPHLA", "Mhcnuggets", "MixMHC2pred", "NetMHCIIpan4.3b", "DeepSeqPanII"]
# ordered_handles = [by_label[m] for m in order if m in by_label]
# ordered_labels = [m for m in order if m in by_label]

# fig.legend(ordered_handles, ordered_labels, loc='lower center', ncol=3, bbox_to_anchor=(0.5, -0.02), frameon=False, fontsize=12)
