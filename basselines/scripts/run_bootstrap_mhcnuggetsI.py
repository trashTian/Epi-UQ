...
def to_mhcnuggets_allele(allele: str) -> str | None:
    if allele is None:
        return None
    ...
    m = re.match(r"^(?:HLA-)?([A-Z0-9]+)\*(\d{2,3}):(\d{2,3})(?::\d{2,3})?$", s)
    if m:
...
def run_mhcnuggets_predict(df_in: pd.DataFrame, peptide_col: str, allele_col: str) -> np.ndarray:
    with tempfile.TemporaryDirectory(prefix="mhcnuggets_bootstrap_") as td:
        td_path = Path(td)
        ...
        for allele, grp in work.groupby(allele_col, sort=False):
            ...
            if exe is not None:
                cmd = [exe, "-c", "I", "-p", str(pep_txt), "-a", str(allele), "-o", str(out_csv)]
            else:
                cmd = [
                    sys.executable,
                    "-m",
            ...
            if len(grp_aff) != len(grp):
                raise RuntimeError(
                    f"MHCnuggets output size mismatch for allele={allele}: "
        ...
        if np.isnan(all_affinity).any():
            ...
            raise RuntimeError(f"MHCnuggets returned NaN/empty scores for {n_nan} rows.")
        return all_affinity


def run_mhcnuggets_predict_panel(peptides: list[str], alleles: list[str]) -> np.ndarray:
    with tempfile.TemporaryDirectory(prefix="mhcnuggets_bootstrap_panel_") as td:
        td_path = Path(td)
        ...
        for i, allele in enumerate(alleles, start=1):
            ...
            if score_col is not None:
                ...
                raise RuntimeError(f"Cannot find prediction score column for allele={allele}. Columns={list(pred.columns)}")

            if len(aff) != len(peptides):
                raise RuntimeError(
                    f"MHCnuggets output size mismatch for allele={allele}: pred={len(aff)} vs peptides={len(peptides)}"
                )
            min_affinity = np.minimum(min_affinity, aff)
            if i % 20 == 0 or i == len(alleles):
                print(f"[INFO] processed alleles: {i}/{len(alleles)}")

        return min_affinity


def main() -> None:
    p = argparse.ArgumentParser(description="MHCnuggets inference + bootstrap metrics")
    ...
    p.add_argument("--default_allele", type=str, default="HLA-A02:01")
    ...
    if use_panel:
        ...
        panel = [to_mhcnuggets_allele(a) for a in panel]
        panel = [a for a in panel if a is not None]
        panel = list(dict.fromkeys(panel))
        if args.max_alleles > 0:
            panel = panel[: args.max_alleles]
        if not panel:
            raise ValueError("No valid alleles in allele panel after MHCnuggets conversion.")
        print(f"[INFO] panel mode enabled. using {len(panel)} alleles for aggregation={args.aggregate}")
    else:
        raw_n = len(work)
        work[allele_col] = work[allele_col].map(to_mhcnuggets_allele)
        work = work.dropna(subset=[allele_col]).reset_index(drop=True)
        dropped = raw_n - len(work)
        if dropped > 0:
            print(f"[INFO] dropped rows with invalid alleles for MHCnuggets: {dropped}")

    if len(work) == 0:
        raise ValueError("No valid rows after filtering and allele conversion.")
    ...
    per_run_path = args.out_dir / "MHCnuggets_bootstrap_per_run.csv"
    summary_path = args.out_dir / "MHCnuggets_bootstrap_summary.csv"
    txt_path = args.out_dir / "MHCnuggets_bootstrap_summary.txt"
